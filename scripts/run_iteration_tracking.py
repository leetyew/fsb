#!/usr/bin/env python
"""
Iteration tracking experiment for Figure 2 replication.
...
Usage:
    ...
    python scripts/run_iteration_tracking.py --seed 42 --track-every 10
    python scripts/run_iteration_tracking.py --n-seeds 3

Results saved to experiments/tracking_{timestamp}/.
"""
...
def run_tracking_trial(
    ...
    print(f"\n  Seed {seed}: Running baseline loop with tracking...")
    ...
    D_a_base, D_r_base, _, baseline_model, baseline_history = loop_base.run(
        holdout=holdout, eval_cfg=eval_cfg, track_every=track_every
    )

    print(f"  Seed {seed}: Running BASL loop with tracking...")
...
def main():
    parser = argparse.ArgumentParser(
        description="Run iteration tracking experiment for Figure 2"
    )
    parser.add_argument(
    ...
    exp_name = f"tracking_{timestamp}"
    if args.name:
    ...
    print("=" * 70)
    print("Iteration Tracking Experiment")
    print("=" * 70)
    print(f"  Output: {exp_dir}")
    print(f"  Seeds: {seeds}")
    print(f"  n_periods: {loop_cfg.n_periods}")
    print(f"  track_every: {track_every}")
    print(f"  n_bayes_samples: {n_bayes_samples}")

    # Run trials
    trials = []
    for seed in tqdm(seeds, desc="Seeds"):
        trial = run_tracking_trial(
            seed, data_cfg, model_cfg, loop_cfg, basl_cfg, eval_cfg, track_every
        )
        trials.append(trial)
