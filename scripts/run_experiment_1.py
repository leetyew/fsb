#!/usr/bin/env python
"""
CLI entrypoint for Experiment I (Table 2).

Runs evaluation accuracy experiment over all replicates and saves
replicate-level results. Aggregation happens in notebooks only.

Usage:
    python scripts/run_exp1.py --data-dir synthetic_runs/my_run
    python scripts/run_exp1.py --data-dir synthetic_runs/my_run --config configs/exp1.yaml
"""
...
from src.experiments.exp1_runner import run_exp1


def main():
    parser = argparse.ArgumentParser(description="Experiment I: Evaluation Accuracy")
    ...
    # Setup output
    run_name = args.run_name or datetime.now().strftime("exp1_%Y%m%d_%H%M%S")
    output_dir = Path(args.output_dir) if args.output_dir else PROJECT_ROOT / "experiments" / run_name
    output_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 70)
    print("Experiment I: Evaluation Accuracy (Table 2)")
    print("=" * 70)
    ...
    bay_cfg = cfg.get("bayesian", {})
    bayesian_cfg = BayesianEvalConfig(
        n_bands=bay_cfg.get("n_bands", 10),
        j_min=bay_cfg.get("j_min", 100),
        j_max=bay_cfg.get("j_max", 10000),
        epsilon=bay_cfg.get("epsilon", 1e-6),
        random_seed=args.seed,
        abr_range=tuple(bay_cfg.get("abr_range", [0.2, 0.4])),
    )
    ...
    # Save results as CSV
    output_file = output_dir / "exp1_results.csv"
